...
const CalculatorConstants = require('../module/dispatcher/CalculatorConstants');

const CHANGE_EVENT = 'change';
let count = 1;

let _signKeyTyped = null;
let _symbolKeyTyped = null;
let _displayScreen = '0';
let _displayFormulae = [];
let _totalNumberOfDigits = 12;
let _exponentialNumberOfDigits = 5;
let _numberKeyPressedBuffer = [];
let _backKeyPressedInARowBuffer = 0;
let _numbersFromBuffer = [];
let _lastCalculation = {};
let _lastPressedWasEqual = false;

const ConsoleEventEmitter = Object.assign({}, EventEmitter.prototype, {
    getDisplayScreen: ()=> {
...
processKey = (keyType, keyValue) => {
    console.log('processKey:' + keyType + "," + keyValue);
    if (keyType === 'number') {
        if (keyValue === '+-') {
...
/** handle number .0123456789**/
processNumberKeyPressed = (keyType, keyValue)=> {
...
ConsoleEventEmitter.dispatchToken = AppDispatcher.register(function (action) {
    switch (action.type) {
...
export default ConsoleEventEmitter;
module.exports = ConsoleEventEmitter;